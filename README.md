...
> **The single, canonical, priority-ranked list of ALL planned work now lives in [`modunote/BACKLOG.md`](modunote/BACKLOG.md)** — features, bugs, cleanup, ops, and open decisions, each with a stable `MN-##` id. It supersedes the old per-section `P1–P5` list that used to live here. The spec docs hold the *how* for items already in the backlog (`PHASE_12_PLAN.md` for AI stages, `UI_POLISH_PLAN.md` for the UI queue, `SUPABASE_MIGRATION_PLAN.md` for the migration); `BUGS.md` holds bug repro/fix detail.

**Current focus (Chunk A, in order):** `python-jose` security bump → activate Sentry in prod → docs truth audit → **CI/CD pipeline** (GitHub Actions gate → release-signed APK → Firebase App Distribution). See `BACKLOG.md` for the full ranked list and `NEXT_THREAD.md` for executor briefs.
