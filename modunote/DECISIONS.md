...
- **Sync triggers: plan default.** Indexable = a note carrying any of `RAG_INDEX_TAGS = {study, notes, research}`. Index (upsert) on the existing save-close hook; deindex when a note loses all trigger tags or is deleted. Constant mirrored Flutter-side (`AppConstants`) and backend-side (config).
