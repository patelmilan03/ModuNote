...
- 📌 **ALL WORK LANDS ON `main`, NO MATTER WHICH BRANCH OR WORKTREE THE SESSION WAS LAUNCHED FROM — ABSOLUTE RULE.**

  Claude Code worktrees are created by the harness and are **inevitable** — a session may open in `modunote/.claude/worktrees/<name>` on a `claude/*` branch with no say in the matter. **Do not refuse to work because of this, and do not stall.** A worktree is an acceptable *scratch working directory*; it is never an acceptable place to *store work*.

  - **The main tree is the one source of truth on disk:** `C:\Milan\Code\resume projects\ModuNote`, with `main` checked out. **Every project file you READ and every project file you WRITE must be addressed by absolute path into that tree**, regardless of your cwd. All file tools (`Read`, `Write`, `Edit`, `Glob`, `Grep`) accept absolute paths — use them. This needs **no git commands at all**: writing into the main tree's working copy is a plain file write, so the change shows up on `main` in GitHub Desktop exactly like a hand edit, and Milan commits it as usual.
  - **Never READ a project file from inside `.claude/worktrees/...`.** A worktree is a snapshot taken when it was created, so it drifts behind `main`, and it is **missing every gitignored file** — `firebase_options.dart`, all `*.g.dart`, `NEXT_THREAD.md`, `session_context.md`, `TECH_STACK.md`, and the entire `modunote-api/` backend. Reading from it yields confidently wrong answers about the current state of the project.
  - **Never WRITE a project file into the worktree.** Those edits land on a throwaway `claude/*` branch that never appears in GitHub Desktop — the developer will never see them, so they are effectively lost.
  - **Run every gate from the main tree.** `cd "C:\Milan\Code\resume projects\ModuNote\modunote"` first, then `flutter analyze` / `dart run custom_lint` / `flutter test` / `dart run build_runner build`. Run from a worktree they either fail on missing generated code or — worse — pass against stale sources and report a green that means nothing.
  - **Stay current with `main` before you start.** At session start, check the main tree, not your cwd:
    ```bash
    git -C "C:/Milan/Code/resume projects/ModuNote" status --porcelain
    git -C "C:/Milan/Code/resume projects/ModuNote" log --oneline -3
    ```
    Uncommitted changes there are **real, current work from a previous session** that the worktree will not contain. Reason about that state, not the worktree's. If it conflicts with what you were about to do, ask before overwriting.
  - **Still no git state changes.** Do not run `commit` / `push` / `pull` / `reset` / `checkout` / `merge` / `rebase` / `rm --cached` — including any attempt to "sync" a `claude/*` branch onto `main`. Landing work on `main` is achieved purely by *where you write the file*, never by a git operation. Reading git (`status`, `log`, `diff`, `ls-files`, `check-ignore`, `worktree list`) is fine.
  - **Never opt *into* a worktree yourself:** no `git worktree add`, and never pass `isolation: "worktree"` to an `Agent` or `Workflow` call. Parallelise multi-agent work by **exclusive file ownership** — each agent gets a disjoint set of files inside the main tree (see `NEXT_THREAD.md` → "PARALLEL EXECUTION CHARTER"). Being *placed* in a worktree by the harness is fine; *choosing* another one is not.
  - **Concurrency caution:** if several sessions write into the main tree at once, two of them touching one file will clobber each other silently. Prefer disjoint files, and always state which files you touched.
  - **Why:** as of 2026-07-25 the harness had produced **2 worktrees and 10 `claude/*` branches, every one with 0 unique commits** — all the actual work had correctly landed on `main`. That is the desired end state: the branches are harmless noise, and the work is where the developer can see it. Periodic pruning is backlog **MN-70** (developer-run: `git worktree remove <path>` → `git worktree prune` → `git branch -D <claude/*>`; Claude only prints these).
