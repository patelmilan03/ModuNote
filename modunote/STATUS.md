...
**Current state (as of 2026-07-02):** Phase 12 **Stage 1 + Stage 2 COMPLETE** (Groq AI writing assistant + RAG QnA). Since then a large "accounts + security + demo" arc landed: **Google Sign-In + auth-gated router + Firestore cloud backup/restore** of notes/tags/categories (hierarchy preserved), a **profile avatar + sign-out**, **per-user RAG isolation** (Firebase ID-token/JWKS verification — fixed a cross-user data leak), the first **automated test suites** (Flutter + backend pytest), an **audio-recording fix**, dead-code cleanup, and a **login-free web portfolio demo** with a **pre-seeded, read-only RAG dataset**. `flutter analyze` = **0**; Flutter **72 tests** + backend **26 pytest** green; `flutter build web` compiles. Live web demo: **https://modunote-ba654.web.app** (redeployed — the current build with demo notes + "Ask your notes" is live; see the 2026-07-08 live check below: **QnA currently broken — Groq API key expired on Render**).

### Session recap — 2026-06-29 → 07-02 (accounts + security + web demo)
- **Test suite** (committed `fa3fefd`): Flutter models / view-models (mocktail + `ProviderContainer`) / local repos (in-memory Drift + a Windows `sqlite3` test bootstrap in `test/util/sqlite3_test_setup.dart`) / `RemoteNoteService` (`http` `MockClient`); backend pytest (`_chunk_text`, `_parse_tags`, endpoints w/ mocked services). Dev-deps: `mocktail`, `sqlite3`, `archive`, `pytest`(+asyncio).
- **Google auth + durable cloud sync**: `services/auth/firebase_auth_service.dart` (Google + anonymous escape hatch + signOut + `authStateChanges`), `services/sync/cloud_sync_service.dart` (atomic restore in a Drift transaction, tags de-duped **by name** + id remap, join rows pruned/rebuilt, orphan-category guard, epoch merge fallback, backup chunked ≤450/batch), branded `views/auth/login_screen.dart`, auth-gated `app_router` (`GoRouterRefreshStream`; local-only if Firebase down; **`kIsWeb` bypass** for the web demo), `main.dart` (no silent anon). `note_list_screen` profile avatar → Google photo (initials/person-icon fallback) + account sheet w/ Sign-out (**guarded when Firebase uninitialised**, e.g. web — see grey-screen fix). Hardened after a read-only audit found a lockout **BLOCKER** + restore data-loss **HIGH**s.
- **Per-user RAG isolation** (fixed a cross-user leak): backend `core/auth.verify_firebase_token` verifies Firebase RS256 ID tokens vs Google JWKS (aud/iss/exp/kid, fail-closed 401); `rag_service.answer_question` filters `Document.user_id == uid`, `_delete_note`/`deindex_note` owner-scoped; routers thread the verified uid. Flutter `RemoteNoteService` sends `Authorization: Bearer <idToken>`. Removed the vestigial static `API_KEY` / `X-API-Key`.
- **Audio fix**: `_stopRecording` saves the clip **immediately**; the Whisper fallback is bounded by a **20 s timeout** and patched in via `AudioEditorViewModel.updateTranscription` (root cause: an unreachable-backend transcribe call hung before the clip was saved).
- **Navbar**: the experimental **liquid-glass** nav was built + tuned, then **reverted** to the solid `MNBottomNav` ("didn't look good") — widget + `liquid_glass_renderer` dep deleted.
- **Web demo**: `kIsWeb` auth bypass; `core/demo/demo_seeder.dart` (6 content-creator demo notes + tags, seeded into local Drift **on web only**, watched in `app.dart`); backend **public read-only** `POST /api/v1/qna/demo` (scoped to `demo_user_id`, no write path); `../modunote-api/scripts/seed_demo.py` (one-time Supabase seed, KEEP IN SYNC with `demo_seeder.dart`). Fixed the web **grey-home-screen** crash (unguarded `FirebaseAuth.instance` in the avatar; `lib/firebase_options.dart` has **no web config**, so Firebase is uninitialised on web).
- **Resume**: `C:\Milan\0Important documents\resume\Milan_Patel_ModuNote_Resume.tex` — full resume in the developer's LaTeX template, ModuNote as the lead project.
- **Supabase migration**: `SUPABASE_MIGRATION_PLAN.md` (**APPROVED**, phased S1–S4, decisions locked) + `supabase/schema.sql` (notes/tags/categories tables + owner-only RLS drafted). **S1 not started** — blocked on developer Supabase console setup.

### ⏳ DEPLOY PENDING (developer — makes the RAG fix + web demo live)
1. **Render** (`../modunote-api`): commit + push; ensure `DEV_MODE=false` + `FIREBASE_PROJECT_ID=modunote-ba654` set; redeploy (adds `/qna/demo` + token auth). Keep-warm `/health` pinger defeats cold starts.
2. **Supabase**: `truncate table chunks, documents;` (clear the old shared `dev-user-local` data), then from the api repo root `python scripts/seed_demo.py` (seeds the demo dataset under `demo`).
3. **Firebase console** (`modunote-ba654`): enable **Google + Anonymous** sign-in; add debug SHA-1 `6E:C1:87:73:91:2E:84:7C:2B:AC:3E:BB:5F:D1:4A:65:64:DB:C4:08` under `com.example.modunote`; download `google-services.json` → `android/app/`; publish `firestore.rules`.
4. **Web redeploy**: `flutter build web --dart-define=API_BASE_URL=https://<render>/api/v1` → `firebase deploy --only hosting` (login-free demo with a working "Ask your notes").
5. **Device**: turn OFF Developer options → "Verify apps over USB" so `flutter run` stops uninstalling/wiping; uninstall once to clear the stale anon session so the login screen appears.

### 🔍 Live check — 2026-07-08 (web demo QnA) — ✅ RESOLVED
Browser-tested https://modunote-ba654.web.app: the redeploy is live (demo notes seeded, "Ask your notes" card + `/qna` render; `main.dart.js` compiled with `API_BASE_URL=https://modunote-api.onrender.com/api/v1`). Two stacked faults were found and fixed same-day:
1. **Expired Groq API key on Render** — `/qna/demo` returned 502 (`Groq 401 expired_api_key`). Developer rotated `GROQ_API_KEY` in the Render dashboard → backend verified healthy (200 + grounded answer + citations; CORS preflight allowlists the web origin).
2. **Stale service-worker cache in the developer's browser** — after the key fix the UI still failed *instantly* (request blocked before the network → symptom of a cached old build). Hard refresh (Ctrl+Shift+R) fixed it; **the live demo now answers end-to-end**. Debugging playbook: `curl /health` then `curl -X POST …/qna/demo` and read the `detail` field — it names the failing provider (Groq / Jina / DB).
...
### 🚦 ACTIVE EXECUTION PLAN — 2026-07-21, Chunk B added 2026-07-25 (READ THIS FIRST ON RESUME)

**Chunk A: four tasks, run in order, ONE task per thread. Chunk B: five cleanups, THREE SUBAGENTS IN PARALLEL in one thread.** The full self-contained briefs for both live in **`NEXT_THREAD.md`** at the **outer repo root** (`ModuNote/NEXT_THREAD.md`) — that file is **gitignored**, so it is invisible to `git` and absent from a fresh clone. Open it directly by path; do not assume a file listing will surface it. Paste the relevant brief verbatim as the first message of the executing thread.

> **Chunk B (MN-05 → MN-09) is the next agent work.** Behaviour-neutral cleanups (`.gitignore` rewrite, dead re-export shim, stale `.gitkeep`s + a stray `{android` dir, dart2js byproducts) parallelised by **exclusive file ownership** — three agents, disjoint file sets, gates run once by the dispatcher afterwards. See `NEXT_THREAD.md` → "EXECUTION PLAN — Chunk B" for the dispatcher brief, the three agent briefs, the ownership table, and Milan's two manual `git rm --cached` steps (MN-07, MN-08). **Every agent addresses files by absolute path into the main tree and runs no gates itself**; harness-created worktrees are fine as a scratch cwd but never a place work is stored (`CLAUDE.md` → "ALL WORK LANDS ON `main`"). Do not pass `isolation: "worktree"` to any Agent call.
...
| 4 | CI/CD — **Landing A** (quality gate, 1 secret) then **Landing B** (signed release → Firebase App Distribution) | 🟢 both jobs green end-to-end; pending Milan's device sign-in confirmation to fully close |

**2026-07-25 — MN-04 first CI run failed, fix applied:** `.github/workflows/ci.yml` pinned Flutter `3.22.0`, whose bundled Dart SDK (3.4.0) is below what `skeletonizer ^2.1.3` requires (Dart ≥3.7.0 — added 2026-06-27, never surfaced locally because the dev machine's Flutter is newer). Fixed: both CI jobs bumped to `flutter-version: '3.41.2'`; `pubspec.yaml`'s `environment.sdk` floor corrected `>=3.3.0` → `>=3.7.0` to match reality. Verified locally with 3.41.2 end-to-end before handing back: `flutter analyze` 0, `dart run custom_lint` clean, `flutter test` **+72 passed, 0 skips**. Note: the Project Identity table's "Min Flutter 3.22.0" (line 18) is now stale too — flagged for the MN-03 docs audit rather than guessed here.

**2026-07-25 — MN-04 second CI run failed, fix applied:** the "Custom lint" step failed even though `dart run custom_lint` itself printed `No issues found!`. Root cause: the gate script's `grep -qv "No issues found"` matches (and triggers the fail branch) whenever **any single output line** lacks that exact phrase — which is always true, since `custom_lint` prints `Analyzing...` and a blank line before its result, so the gate could never pass. Fixed to `if ! echo "$OUTPUT" | grep -q "No issues found"` (fails only when the phrase is absent from the whole output); verified both the pass case (real captured output) and a synthetic fail case against the corrected logic locally.

**2026-07-25 — MN-04 Landing A green, Landing B failed, fix applied (correction to the entry above):** Quality Gate now passes. The Release job failed signing: `KeytoolException: ... keystore password was incorrect` on `/tmp/modunote-release.jks`. A same-day pass on this file called it a pure secrets mismatch with no code bug — that was wrong, or at least incomplete. Re-examined `ci.yml`'s "Write key.properties" step and found a real bug: it writes `KEYSTORE_PASSWORD`/`KEY_PASSWORD`/`KEY_ALIAS` into an **unquoted heredoc** (`<< EOF`), and bash performs `$`-expansion + backtick command-substitution inside unquoted heredoc bodies. A password containing `$` (common in generated strong passwords, e.g. `Sup3r$ecret99`) silently truncates to `Sup3r` before it ever reaches `keytool`. Reproduced the exact truncation locally with a demo, fixed by quoting the delimiter (`<< 'EOF'` — disables shell reinterpretation of the body; GitHub's `${{ secrets.X }}` substitution already happened before bash runs, so it's unaffected), then re-verified the fix preserves a `$`-containing value byte-for-byte. The other two secret-writing steps in the same job were re-checked and are already safe (base64 has no shell metacharacters; the JSON step was already single-quoted). **Milan still needs to:** commit + push `.github/workflows/ci.yml`. If the release job still fails after this, it's genuinely a secrets-side mismatch — confirm `keytool -list -v -keystore C:\Milan\keys\modunote-release.jks -alias modunote` opens locally, then re-paste `KEYSTORE_PASSWORD`/`KEY_PASSWORD` fresh.

**2026-07-25 — MN-04 keystore fix worked, Landing B advanced further and hit a new failure, fix applied:** the release job got past signing this time and built the APK successfully (`✓ Built build/app/outputs/flutter-apk/app-release.apk (68.2MB)`), then crashed immediately after: `--build-number=5: command not found`, exit 127. Root cause: the "Build release APK" step's `--dart-define=API_BASE_URL=${{ secrets.API_BASE_URL }} \` was the only unquoted `${{ }}` substitution in the file sitting directly before a line-continuation backslash — everywhere else in `ci.yml` wraps these in `"..."`. GitHub Actions substitutes `${{ }}` as raw text before bash parses the script, so a stray trailing newline in the `API_BASE_URL` secret (plausible from a copy-paste) gets injected mid-script and silently ends the command early — which is exactly consistent with the build succeeding with `--release`/`--dart-define` applied, then `--build-number=...` running as its own bogus command. Fixed by collapsing the step to one line with the value quoted: `flutter build apk --release --dart-define=API_BASE_URL="${{ secrets.API_BASE_URL }}" --build-number=${{ github.run_number }}`. Audited the whole file for the same unquoted-substitution-before-continuation pattern — no other instance. **Milan still needs to:** commit + push `.github/workflows/ci.yml`; also worth re-checking the `API_BASE_URL` secret itself for trailing whitespace/newline and re-setting it cleanly, since a stray newline baked into the compiled app constant could outlive this CI fix.

**2026-07-25 — MN-04 quoting fix wasn't enough, hardened defensively:** with the value quoted, the crash changed shape — `flutter build apk` now errors `Target file "--dart-define=API_BASE_URL=***\n" not found.` This confirms the `API_BASE_URL` secret carries an embedded/trailing newline: quoting kept it inside one argv token, but that token now contains a raw newline that flutter's arg parser doesn't recognise as `--dart-define=...`, so the whole string falls through to the positional "target file" slot instead. Rather than keep chasing the exact shape of the corruption, hardened the step: moved the secret to `env:` (env vars pass at the OS level, immune to raw `${{ }}` text-substitution corrupting the script) and strip all whitespace with `tr -d '[:space:]'` before use, plus a length-only debug line (never the value). Verified locally with a synthetic embedded-newline value — cleans correctly, forms exactly 2 argv tokens. **Milan still needs to:** commit + push `.github/workflows/ci.yml`; separately worth re-pasting the `API_BASE_URL` secret cleanly at the source, though the workflow no longer depends on it being clean.

**2026-07-25 — MN-04 CI/CD green end-to-end for the first time.** Quality Gate + Release job both passed; a signed release APK was built and distributed via Firebase App Distribution to the `milan` tester group. Took 4 rounds of CI debugging across this session (Flutter/Dart SDK floor mismatch with `skeletonizer`, an always-fail `custom_lint` gate script bug, a `$`-truncated keystore password from an unquoted heredoc, and an `API_BASE_URL` secret with embedded/trailing whitespace) — see the dated entries above for each root cause and fix. **One acceptance check remains:** confirm Google Sign-In works in the installed release APK — this is the one thing CI builds/signs but can't itself verify; it's the actual proof the release SHA-1 registered during setup took effect. MN-04 stays 🟡/in-progress in `BACKLOG.md` until that's confirmed, then flip to ✅ and remove **P1** from the README Roadmap per the CLAUDE.md roadmap rule.

**Note (2026-07-25):** the Firebase App Tester Android app is no longer listed on the Play Store — it's now sideloaded directly via a browser prompt at `appdistribution.firebase.google.com` (or the link in the App Distribution invite email), not installed from the Play Store as B-PREREQ step 8 in `NEXT_THREAD.md` originally assumed.
...
- Also 2026-07-08: **git recovery** (session work was orphaned on a detached-HEAD commit `0bd26b0`; fast-forwarded `main` onto it, deleted 6 stale `claude/*` branches + 3 stale `.claude/worktrees` — all verified ancestors of main; developer pushed). **Web redeployed** (error-source UI live). **RLS enabled** on `documents`/`chunks`/`alembic_version` via the Supabase MCP connector (no policies = deny-by-default for the PostgREST/anon path; the backend's `postgres` role bypasses — `/qna/demo` verified 200 after). Supabase advisors: only expected INFO lints + a cosmetic `vector`-extension-in-public WARN.
- ⏸️ **Stage 3 PAUSED here (2026-07-08, developer call)** after the Sentry step — resuming later with Langfuse tracing, then RAGAS. Next active item: the **efficiency / de-bloat pass** (README Roadmap P2 / `UI_POLISH_PLAN.md` item 4).

### Efficiency / de-bloat pass — rounds 2+3 (2026-07-08): note_list + settings splits ✅ · gitignore trap fixed 🔴
- **Round 2:** `note_list_screen.dart` **1,137 → 137 lines** (keeps `NoteListScreen` + `_DataBody`); widgets moved verbatim to `views/note_list/widgets/` — `ask_notes_card` (AskNotesCard), `swipeable_note_card` (SwipeableNoteCard + private bg/actions-sheet/tile), `filter_chip_bar` (FilterChipBar + private chip), `app_bar_section` (AppBarSection + private _ProfileAvatar), `section_header` (SectionHeader), `note_list_states` (LoadingBody/ErrorBody/FilteredEmptyState/EmptyState + private _SkeletonBox).
- **Round 3:** `settings_screen.dart` **918 → 53 lines** (keeps `SettingsScreen` only); widgets moved verbatim to `views/settings/widgets/` — `rag_tags_card` (RagTagsCard + private chips/_TagPickerSheet), `appearance_card` (AppearanceCard + private tiles/previews/radio), `voice_notes_card`, `settings_app_bar`, `archive_card`.
- Verified: `flutter analyze` = 0; 72 tests green (incl. the scope-picker widget test driving the extracted `_TagPickerSheet`); scripted git-diff confirmed every moved class **byte-identical** vs HEAD after renames/`super.key` (screen bodies identical too).
- 🔴 **gitignore trap found & fixed:** the root `.gitignore` had a blanket `/modunote` line — tracked files worked, but ALL new files under `modunote/` were invisible to git (the new widget dirs, and retroactively `lib/core/demo/` + `supabase/`, which were **never on GitHub** — a fresh clone couldn't build the web demo). Removed `/modunote` (+ a duplicate line); the inner `modunote/.gitignore` still covers all secrets (verified: firebase_options / google-services / session_context / dart_defines / *.g.dart stay hidden). **Developer: the next commit MUST include `.gitignore`, the 3 `widgets/` dirs, `lib/core/demo/`, and `supabase/`.**

### Efficiency / de-bloat pass — round 1 (2026-07-08): editor split ✅
Scope locked with the developer: **code structure only · editor only · pure refactor (zero behavior change)**. `note_editor_screen.dart` **2,697 → 944 lines**: the 15 private widget classes moved verbatim into `lib/presentation/views/note_editor/widgets/` — `voice_panel.dart` (VoicePanel), `editor_app_bar.dart` (EditorAppBar + private _CircleIconButton/_SaveBadge), `recording_overlay.dart` (RecordingOverlay + private _WaveformBars/_PulsingStopButton), `note_options_sheet.dart` (NoteOptionsSheet + **OptionsRow**, public — shared with the AI sheet), `tag_suggest_banner.dart` (TagSuggestBanner), `ai_tools_sheet.dart` (AiToolsSheet + private _AiAction/_kAiActions/_AiSheetButton), `tag_input_sheet.dart` (TagInputSheet + private tiles). Only changes beyond the moves: public renames, `super.key` added, import pruning (`flutter/services`, `audio_pref_view_model` left the screen file). `flutter analyze` = 0; 72 tests green; read-only audit **ALL PASS incl. a git-diff verbatim-move check**. Possible follow-up rounds (not started): same split for `note_list_screen.dart` (1,137) and `settings_screen.dart` (918); deps/build-size audits were explicitly descoped this round.

### NEXT (roadmap)
The canonical, prioritized roadmap now lives in the root **`README.md` → "Roadmap"** — the ONLY place proposed/future features are recorded (rule in `CLAUDE.md`; consolidated 2026-07-08; **re-prioritized 2026-07-14** per the cross-project plan in `resume projects/PORTFOLIO_PLAN.md`). Agreed order: **CI/CD pipeline (GitHub Actions + Firebase App Distribution)** → splash/onboarding → Stage 3 remainder (Langfuse → RAGAS) → Stage-4 leftovers → Supabase migration S1–S4 last. De-bloat remainder (deps/build-size audit) parked.
- **Near-term developer ops:** ~~redeploy web~~ ✅ · ~~enable RLS~~ ✅ (both done 2026-07-08) · set `SENTRY_DSN` in Render (see Stage 3 above) · Resume `.tex`: fill the GitHub URL + "Live" link.
- **Notes for a new thread:** `com.example.modunote` is the REAL package (docs' `com.modunote.app` is aspirational/stale). `lib/firebase_options.dart` is Android-only (no web config) → web runs Firebase-free by design. `session_context.md` appears **developer-edited/diverged** — trust git history + this STATUS block over it.
...
- ✅ **S2-F10 — Bulk re-index + app-wide toasts** *(2026-06-27)*: (a) "Re-index all notes now" button in `_RagTagsCard` → `RagReindex` (`@riverpod`, `rag_reindex_view_model.dart`) iterates active notes, indexes those whose tags ∈ scope, returns `(ok, fail)`; button shows a spinner while running. (b) Global toast infra: `toastification` package + `ToastificationWrapper` in `app.dart` + `rootNavigatorKey` (in `core/utils/app_toast.dart`, passed to GoRouter) so background ops toast from anywhere; helpers `showSuccessToast`/`showErrorToast`/`showInfoToast`. (c) Auto on-close sync now toasts **only on INDEX failure** (deindex/cleanup stays silent); re-index button toasts a success/fail/none summary. `flutter analyze` = 0; verification ALL PASS. NOTE: re-index will report failures until the Render→Supabase pooler fix lands.
- ✅ **S2-F11 — Skeleton loaders** *(2026-06-27)*: `skeletonizer` package + `presentation/widgets/mn_skeletons.dart` (`skeletonNotes`/`skeletonTags`/`MNSkeletonNoteList`). Replaced the plain `CircularProgressIndicator` loading states on **Tags** (skeleton tag rows), **Search** + **Archive** (skeleton note cards), and the **Note editor** (bespoke `_buildSkeletonEditor`). `flutter analyze` = 0; verification ALL PASS. **Render DB fix verified working 2026-06-27** (live `/qna` + `/index` now 200). Added `.vscode/launch.json` (two run configs: Render-API / local).
...
> **Build queue:** ✅ skeleton loaders → ✅ voice panel (Variant A) → ✅ test suite. Remaining items and their order now live in the root `README.md` → "Roadmap" (2026-07-08: Stage 3 first, then de-bloat → startup UX). Specs stay in `UI_POLISH_PLAN.md`.
