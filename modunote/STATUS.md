...
### NEXT (roadmap)
The canonical, prioritized roadmap now lives in the root **`README.md` → "Roadmap"** — the ONLY place proposed/future features are recorded (rule in `CLAUDE.md`; consolidated 2026-07-08). Summary of the agreed order: **Stage 3 observability (Sentry first)** → de-bloat pass → splash/onboarding → Stage-4 leftovers → Supabase migration S1–S4 (parked, low priority).
- **Near-term developer ops:** redeploy web (`flutter build web --dart-define=API_BASE_URL=https://modunote-api.onrender.com/api/v1` → `firebase deploy --only hosting`) to ship the QnA error-source UI; run `alter table documents enable row level security; alter table chunks enable row level security;` in the Supabase SQL Editor (no policies — backend/postgres role bypasses; blocks the PostgREST/anon path). Resume `.tex`: fill the GitHub URL + "Live" link.
