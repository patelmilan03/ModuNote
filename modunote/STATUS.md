...
- **Next in Stage 3:** Langfuse tracing on all Groq + embedding call sites, then the RAGAS eval dataset + baseline scores.

### NEXT (roadmap)
The canonical, prioritized roadmap now lives in the root **`README.md` → "Roadmap"** — the ONLY place proposed/future features are recorded (rule in `CLAUDE.md`; consolidated 2026-07-08). Summary of the agreed order: **Stage 3 observability (in progress above)** → de-bloat pass → splash/onboarding → Stage-4 leftovers → Supabase migration S1–S4 (parked, low priority).
