...
**2026-07-25 — MN-04 Landing A green, Landing B failed (blocked on Milan):** Quality Gate now passes. The Release job fails signing: `KeytoolException: ... keystore password was incorrect` on `/tmp/modunote-release.jks`. Reviewed `ci.yml:142-153` (keystore decode + `key.properties` write) — logic is correct, not a script bug. This is a secrets mismatch: either `ANDROID_KEYSTORE_BASE64` is stale against a since-regenerated keystore, or `KEYSTORE_PASSWORD`/`KEY_PASSWORD` has a hidden trailing newline from a clipboard paste. Per charter, secret values stay Milan-only — not inspected or requested here. **Milan still needs to:** confirm `keytool -list -v -keystore C:\Milan\keys\modunote-release.jks -alias modunote` opens locally with the password he has, then re-generate all 4 keystore secrets fresh (type passwords directly into GitHub, don't paste) and re-push.
