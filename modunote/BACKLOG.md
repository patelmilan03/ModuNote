...
- 2026-07-25 · MN-04 (blocked on Milan) — Landing A (Quality Gate) now green; Landing B (Release job) failed at `assembleRelease`: `KeytoolException: Failed to read key "modunote" from store "/tmp/modunote-release.jks": keystore password was incorrect`. Reviewed the workflow's keystore-decode + `key.properties`-write steps (`ci.yml:142-153`) — logic is correct, same pattern already proven for the other two decoded secrets in the same job. This is a secrets-side mismatch (stale `ANDROID_KEYSTORE_BASE64` vs a since-regenerated keystore, or a `KEYSTORE_PASSWORD`/`KEY_PASSWORD` secret with a hidden trailing newline from clipboard paste), not a code bug — per charter, secret values are Milan-only, never inspected or requested here. **Milan to do:** verify `keytool -list -v -keystore C:\Milan\keys\modunote-release.jks -alias modunote` succeeds locally with the password he has; if so, re-generate all 4 keystore-related secrets fresh from the current `.jks` (type passwords directly into GitHub rather than pasting) and re-push.
